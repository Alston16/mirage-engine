...
- [ ] T007 [P] In `src/solver.rs` add `pub(crate) fn combine_friction(a: f32, b: f32) -> f32 { (a * b).sqrt() }` with a doc comment explaining why it is not `max`. Add unit tests: symmetric (`combine(0.3, 0.8) == combine(0.8, 0.3)` exactly), `0` if either argument is `0`, `combine(x, x) == x` within 1e-6.
